...
*               to main.c or bst.c.
*****************************************************************
*  1. bst.c, function bst_insert, Add indentation to make more clear what is in the if statement
*  2. bst.c, function bst_delete, Add indentation to nested if statement to make easier to read
*  3. bst.c, function bst_delete, start action of if statement, and else statement on new line to make easier to read and match other formatting, add more lines and { } around action of if statement for readability
*  4. bst.c, function bst_init, add indentation for readability
*  5. bst.c, function bst_min, Add {} and indentation for while loop to make clearer
...
*****************************************************************
*  1. bst.c, fuction bst_erase, decrement changed from += to -= to decrement size of the tree instead of increment
*  2. bst.c, fuction bst_delete, & changed to && to compare whole thing instead of one bit
*  3. bst.c, fuction bst_delete, when traverse all the way to the right for max, the p_swapNode should equal to p_swapNode->right for maximum
*  4. bst.c, fuction bst_delete, in casw 2, when the child is on the right side of the parent, the if conidtion should check if "p_parentNode->right == p_currNod"
****************************************************************/
...
void bst_delete ( bsn_t *p_currNode, bsn_t *p_parentNode)
{
	...
	else
	{
		...
		{
			// need to know whether we are to the left or right of the parent
			if(p_parentNode->right == p_currNode)
			{
